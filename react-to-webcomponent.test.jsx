...
test("works with attributes set with propTypes", async () => {
	...
	await new Promise(r => {
		setTimeout(() => {
			expect(body.firstElementChild.innerHTML).toEqual("<h1>Hello, Christopher</h1>");
			r();
		}, 0);
	});
...
test('It works with dashed attributes styled set to "true"', async () => {
	...
	const MyGreeting = reactToWebComponent(Greeting, React, ReactDOM, { dashStyleAttributes: true })
